...
const OffCanvas: FC<IOffCanvasProps> = ({
	...
	// Backdrop close function (ignore portaled overlays e.g. SweetAlert2 — their DOM is outside this ref)
	const closeCanvas = (event: { target: EventTarget | null }) => {
		...
		// Portaled modals (e.g. FilePreviewModal) sit outside offcanvas DOM; backdrop clicks must not close the panel.
		if (el?.closest?.('.modal') || el?.closest?.('.modal-backdrop')) {
			return;
		}
		// @ts-ignore
		if (!ref.current.contains(target) && !isRightPanel && isBackdrop) {
			setOpen(false);
		}
	};
