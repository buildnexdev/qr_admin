  ...
  return (
    <div className="page-container" style={{
      ...
      <style>{`
        ...
        .btn-add:hover { transform: translateY(-1px); box-shadow: 0 8px 28px rgba(var(--accent-rgb),0.4); }
        ...
        .cat-card:hover {
          ...
          box-shadow: 0 12px 40px rgba(0,0,0,0.4), 0 0 0 1px rgba(var(--accent-rgb),0.1);
        }
                ...
                {cat.tags && cat.tags.length > 0 && (
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginTop: 'auto' }}>
                    {cat.tags.map((tag, tIdx) => (
                      <span key={tIdx} style={{ background: 'rgba(var(--accent-rgb),0.08)', border: '1px solid var(--border)', color: 'var(--amber-lt)', padding: '4px 10px', borderRadius: '20px', fontSize: '0.7rem', fontWeight: 600 }}>
                        {tag}
                      </span>
                    ))}
                    {!cat.status && (
      ...
      <div style={{
        ...
        <div style={{ padding: '24px 32px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button onClick={handleCloseModal} style={{ background: 'rgba(var(--accent-rgb),0.1)', border: 'none', width: '36px', height: '36px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'var(--amber)', transition: 'background 0.2s' }} onMouseOver={(e: any) => e.currentTarget.style.background = 'rgba(var(--accent-rgb),0.2)'} onMouseOut={(e: any) => e.currentTarget.style.background = 'rgba(var(--accent-rgb),0.1)'}>
            <X size={20} />
          </button>
        </div>
